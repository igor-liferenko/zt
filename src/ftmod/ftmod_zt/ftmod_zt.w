...
static __inline__ uint8_t linear_to_ulaw(int linear)
{
	...
	u_val =
	    (uint8_t) (((seg << 4) | ((linear >> (seg + 3)) & 0xF)) ^
		       mask);
			   ...
			   int codec)
{
	...
	for (j = 0;
	     j < (sizeof(g->receive_gain) / sizeof(g->receive_gain[0]));
	     j++) {
	    if (rxgain) {
...
static ftdm_status_t zt_open(ftdm_channel_t * ftdmchan)
{
	...
	int err;
	if ((err =
	     ioctl(ftdmchan->sockfd, DAHDI_SET_BLOCKSIZE, &blocksize))) {
	...
	if (ftdmchan->type == FTDM_CHAN_TYPE_B) {
		...
		snprintf(ftdmchan->last_error,
			 sizeof(ftdmchan->last_error), "%m");
				...
				ftdm_command_t command, void *obj)
{
    ...
    case FTDM_COMMAND_SET_INTERVAL:
	{
	    ...
	    if (!
		(err =
		 ioctl(ftdmchan->sockfd, DAHDI_SET_BLOCKSIZE, &len))) {
		ftdmchan->packet_len = len;
		ftdmchan->effective_interval = ftdmchan->native_interval =
		    ftdmchan->packet_len / 8;
	...
	{
	    err =
		ioctl(ftdmchan->sockfd, DAHDI_GETRXBITS,
		      &ftdmchan->rx_cas_bits);
	    if (!err) {
...
static ftdm_status_t zt_get_alarms(ftdm_channel_t * ftdmchan)
{
	...
	if (ioctl(ftdmchan->sockfd, DAHDI_GET_PARAMS, &params)) {
	    snprintf(ftdmchan->last_error, sizeof(ftdmchan->last_error),
		     "ioctl failed (%m)");
	    snprintf(ftdmchan->span->last_error,
		     sizeof(ftdmchan->span->last_error),
		     "ioctl failed (%m)");
			    ...
			    short *poll_events)
{
	...
	if ((pfds[i - 1].revents & 0x002) || (span->channels[i]->io_data)) {
		...
		span->channels[i]->last_event_time =
...
ftdm_status_t zt_next_event(ftdm_span_t * span, ftdm_event_t ** event)
{
	...
	} else if (ioctl(fchan->sockfd, DAHDI_GETEVENT, &zt_event_id) ==
		   -1) {
	    ftdm_log(FTDM_LOG_ERROR,
		     "[s%dc%d][%d:%d] "
			     ...
			     size_t *datalen)
{
	...
	if (read_errno == ELAST) {
	    zt_event_t zt_event_id = 0;
	    if (ioctl(ftdmchan->sockfd, DAHDI_GETEVENT, &zt_event_id) ==
		-1) {
		ftdm_log(FTDM_LOG_ERROR,
			 "[s%dc%d][%d:%d] "
